...
/**
 * Instantiate a script reader.
 * @returns {ScriptReader}
 */

Script.prototype.read = function read() {
  return new ScriptReader(this);
};

/**
 * Instantiate a script writer.
 * @returns {ScriptWriter}
 */

Script.prototype.write = function write() {
...
Script.prototype[Symbol.iterator] = function iterator() {
  return new ScriptIterator(this, 0);
};
...
Script.prototype.entries = function entries() {
  return new ScriptIterator(this, 1);
};

/**
 * Instantiate a value-only iterator (returns op values).
 * @returns {ScriptIterator}
 */

Script.prototype.ops = function ops() {
  return new ScriptIterator(this, 2);
};
...
Script.prototype.getSubscript = function getSubscript(index) {
  ...
  return Script.fromRaw(this.raw.slice(index));
};
...
Script.prototype.removeSeparators = function removeSeparators() {
  ...
  for (const op of this.ops()) {
    ...
    if (op === opcodes.OP_CODESEPARATOR) {
      found = true;
      break;
    }
  }
...
Script.prototype.indexOf = function indexOf(data) {
  for (const [i, op] of this.entries()) {
    if (op.value === -1)
      break;
...
Script.prototype.isCode = function isCode() {
  for (const op of this.ops()) {
    ...
    if (op > opcodes.OP_CHECKSEQUENCEVERIFY)
      return false;
  }
...
Script.prototype.fromNulldata = function fromNulldata(flags) {
  ...
  script.pushOp(opcodes.OP_RETURN);
  script.pushData(flags);
...
Script.prototype.isPubkey = function isPubkey(minimal) {
  ...
  if (this.raw.length < 35 || this.raw.length > 71)
    return false;

  if (this.raw[this.raw.length - 1] !== opcodes.OP_CHECKSIG)
    return false;

  const script = this.read();
  const size = script.getLength();
...
Script.prototype.isPubkeyhash = function isPubkeyhash(minimal) {
  ...
  if (this.raw.length < 25 || this.raw.length > 29)
    return false;

  if (this.raw[0] !== opcodes.OP_DUP)
    return false;
  ...
  return script.getOp() === opcodes.OP_DUP
    && script.getOp() === opcodes.OP_HASH160
    && script.getLength() === 20
    && script.getOp() === opcodes.OP_EQUALVERIFY
    && script.getOp() === opcodes.OP_CHECKSIG
    && script.left() === 0;
};
...
Script.prototype.getPubkeyhash = function getPubkeyhash(minimal) {
  ...
  return this.read().seek(2).getData();
};
...
Script.prototype.isMultisig = function isMultisig(minimal) {
  if (this.raw.length < 41 || this.raw.length > 1123)
    return false;
  ...
  return script.getSmall() === n
    && script.getOp() === opcodes.OP_CHECKMULTISIG
    && script.left() === 0;
};
...
Script.prototype.isPubkeyInput = function isPubkeyInput() {
  if (this.raw.length < 10 || this.raw.length > 78)
    return false;

  const script = this.read();
  const size = script.getLength();

  return size >= 9 && size <= 73 && script.left() === 0;
};
...
Script.prototype.isPubkeyhashInput = function isPubkeyhashInput() {
  if (this.raw.length < 44 || this.raw.length > 148)
    return false;

  const script = this.read();
  const sig = script.getLength();
  const key = script.getLength();
...
Script.prototype.getPubkeyhashInput = function getPubkeyhashInput() {
  ...
  return [script.getData(), script.getData()];
};
...
Script.prototype.isScripthashInput = function isScripthashInput() {
  ...
  // opcodes.
  if (redeem.raw.length === 0)
    return false;

  if (common.isSignatureEncoding(redeem.raw))
    return false;

  if (common.isKeyEncoding(redeem.raw))
    return false;
...
function ScriptReader(script) {
  assert(Script.isScript(script));
  this.br = new BufferReader(script.raw, true);
  this.index = -1;
}
...
ScriptReader.prototype.offset = function offset() {
  return this.br.offset;
};

ScriptReader.prototype.seek = function seek(offset) {
...
ScriptReader.prototype.read = function read() {
  const op = this.get();
  return [this.index, op];
};

ScriptReader.prototype.get = function get() {
  if (this.br.left() === 0)
    return null;

  this.index += 1;

  return Opcode.fromReader(this.br);
};

ScriptReader.prototype.getOp = function getOp() {
  if (this.br.left() === 0)
    return -1;

  this.index += 1;

  return Opcode.readOp(this.br);
};

ScriptReader.prototype.getData = function getData() {
  const op = this.get();
...
ScriptReader.prototype.getLength = function getLength() {
  if (this.br.left() === 0)
    return -1;

  this.index += 1;

  return Opcode.readLength(this.br);
};

ScriptReader.prototype.getPush = function getPush() {
  const op = this.get();
...
ScriptReader.prototype.getString = function getString(enc) {
  const op = this.get();

  if (!op)
    return null;

  return op.toString(enc);
};

ScriptReader.prototype.getSmall = function getSmall() {
  const op = this.get();
...
ScriptReader.prototype.getNum = function getNum(minimal, limit) {
  const op = this.get();

  if (!op)
    return null;

  return op.toNum(minimal, limit);
};

ScriptReader.prototype.getInt = function getInt(minimal, limit) {
  const op = this.get();

  if (!op)
    return -1;

  return op.toInt(minimal, limit);
};

ScriptReader.prototype.getBool = function getBool() {
  const op = this.get();
...
function ScriptWriter(script) {
  if (!(this instanceof ScriptWriter))
    return new ScriptWriter(script);

  this.script = script || new Script();
  this.code = this.script.toArray();
}

Object.defineProperty(ScriptWriter.prototype, 'length', {
  get() {
...
ScriptWriter.prototype.get = function get(index) {
  if (index < 0)
...
ScriptWriter.prototype.pop = function pop() {
  const op = this.code.pop();
  return op || null;
};

ScriptWriter.prototype.shift = function shift() {
  const op = this.code.shift();
  return op || null;
};

ScriptWriter.prototype.remove = function remove(index) {
  if (index < 0)
    index += this.code.length;
...
ScriptWriter.prototype.set = function set(index, op) {
  if (index < 0)
    index += this.code.length;
...
ScriptWriter.prototype.push = function push(op) {
  assert(Opcode.isOpcode(op));
  this.code.push(op);
  return this;
};

ScriptWriter.prototype.unshift = function unshift(op) {
  assert(Opcode.isOpcode(op));
  this.code.unshift(op);
  return this;
};

ScriptWriter.prototype.insert = function insert(index, op) {
  if (index < 0)
    index += this.code.length;
...
ScriptWriter.prototype.getOp = function getOp(index) {
  const op = this.get(index);
  return op ? op.value : -1;
};

ScriptWriter.prototype.popOp = function popOp() {
  const op = this.pop();
  return op ? op.value : -1;
};

ScriptWriter.prototype.shiftOp = function shiftOp() {
  const op = this.shift();
  return op ? op.value : -1;
};

ScriptWriter.prototype.removeOp = function removeOp(index) {
  const op = this.remove(index);
  return op ? op.value : -1;
};

ScriptWriter.prototype.setOp = function setOp(index, value) {
  return this.set(index, Opcode.fromOp(value));
};

ScriptWriter.prototype.pushOp = function pushOp(value) {
  return this.push(Opcode.fromOp(value));
};

ScriptWriter.prototype.unshiftOp = function unshiftOp(value) {
  return this.unshift(Opcode.fromOp(value));
};

ScriptWriter.prototype.insertOp = function insertOp(index, value) {
...
ScriptWriter.prototype.getData = function getData(index) {
  const op = this.get(index);
  return op ? op.data : null;
};

ScriptWriter.prototype.popData = function popData() {
  const op = this.pop();
  return op ? op.data : null;
};

ScriptWriter.prototype.shiftData = function shiftData() {
  const op = this.shift();
  return op ? op.data : null;
};

ScriptWriter.prototype.removeData = function removeData(index) {
  const op = this.remove(index);
  return op ? op.data : null;
};

ScriptWriter.prototype.setData = function setData(index, data) {
  return this.set(index, Opcode.fromData(data));
};

ScriptWriter.prototype.pushData = function pushData(data) {
  return this.push(Opcode.fromData(data));
};

ScriptWriter.prototype.unshiftData = function unshiftData(data) {
  return this.unshift(Opcode.fromData(data));
};

ScriptWriter.prototype.insertData = function insertData(index, data) {
...
ScriptWriter.prototype.getLength = function getLength(index) {
  const op = this.get(index);
...
ScriptWriter.prototype.getPush = function getPush(index) {
  const op = this.get(index);
  return op ? op.toPush() : null;
};

ScriptWriter.prototype.popPush = function popPush() {
  const op = this.pop();
  return op ? op.toPush() : null;
};

ScriptWriter.prototype.shiftPush = function shiftPush() {
  const op = this.shift();
  return op ? op.toPush() : null;
};

ScriptWriter.prototype.removePush = function removePush(index) {
  const op = this.remove(index);
  return op ? op.toPush() : null;
};

ScriptWriter.prototype.setPush = function setPush(index, data) {
  return this.set(index, Opcode.fromPush(data));
};

ScriptWriter.prototype.pushPush = function pushPush(data) {
  return this.push(Opcode.fromPush(data));
};

ScriptWriter.prototype.unshiftPush = function unshiftPush(data) {
  return this.unshift(Opcode.fromPush(data));
};

ScriptWriter.prototype.insertPush = function insertPush(index, data) {
...
ScriptWriter.prototype.getString = function getString(index, enc) {
  const op = this.get(index);
  return op ? op.toString(enc) : null;
};

ScriptWriter.prototype.popString = function popString(enc) {
  const op = this.pop();
  return op ? op.toString(enc) : null;
};

ScriptWriter.prototype.shiftString = function shiftString(enc) {
  const op = this.shift();
  return op ? op.toString(enc) : null;
};

ScriptWriter.prototype.removeString = function removeString(index, enc) {
  const op = this.remove(index);
  return op ? op.toString(enc) : null;
};

ScriptWriter.prototype.setString = function setString(index, str, enc) {
  return this.set(index, Opcode.fromString(str, enc));
};

ScriptWriter.prototype.pushString = function pushString(str, enc) {
  return this.push(Opcode.fromString(str, enc));
};

ScriptWriter.prototype.unshiftString = function unshiftString(str, enc) {
  return this.unshift(Opcode.fromString(str, enc));
};

ScriptWriter.prototype.insertString = function insertString(index, str, enc) {
...
ScriptWriter.prototype.getSmall = function getSmall(index) {
  const op = this.get(index);
  return op ? op.toSmall() : -1;
};

ScriptWriter.prototype.popSmall = function popSmall() {
  const op = this.pop();
  return op ? op.toSmall() : -1;
};

ScriptWriter.prototype.shiftSmall = function shiftSmall() {
  const op = this.shift();
  return op ? op.toSmall() : -1;
};

ScriptWriter.prototype.removeSmall = function removeSmall(index) {
  const op = this.remove(index);
  return op ? op.toSmall() : -1;
};

ScriptWriter.prototype.setSmall = function setSmall(index, num) {
  return this.set(index, Opcode.fromSmall(num));
};

ScriptWriter.prototype.pushSmall = function pushSmall(num) {
  return this.push(Opcode.fromSmall(num));
};

ScriptWriter.prototype.unshiftSmall = function unshiftSmall(num) {
  return this.unshift(Opcode.fromSmall(num));
};

ScriptWriter.prototype.insertSmall = function insertSmall(index, num) {
...
ScriptWriter.prototype.getNum = function getNum(index, minimal, limit) {
  const op = this.get(index);
  return op ? op.toNum(minimal, limit) : null;
};

ScriptWriter.prototype.popNum = function popNum(minimal, limit) {
  const op = this.pop();
  return op ? op.toNum(minimal, limit) : null;
};

ScriptWriter.prototype.shiftNum = function shiftNum(minimal, limit) {
  const op = this.shift();
  return op ? op.toNum(minimal, limit) : null;
};

ScriptWriter.prototype.removeNum = function removeNum(index, minimal, limit) {
  const op = this.remove(index);
  return op ? op.toNum(minimal, limit) : null;
};

ScriptWriter.prototype.setNum = function setNum(index, num) {
  return this.set(index, Opcode.fromNum(num));
};

ScriptWriter.prototype.pushNum = function pushNum(num) {
  return this.push(Opcode.fromNum(num));
};

ScriptWriter.prototype.unshiftNum = function unshiftNum(num) {
  return this.unshift(Opcode.fromNum(num));
};

ScriptWriter.prototype.insertNum = function insertNum(index, num) {
...
ScriptWriter.prototype.getInt = function getInt(index, minimal, limit) {
  const op = this.get(index);
  return op ? op.toInt(minimal, limit) : -1;
};

ScriptWriter.prototype.popInt = function popInt(minimal, limit) {
  const op = this.pop();
  return op ? op.toInt(minimal, limit) : -1;
};

ScriptWriter.prototype.shiftInt = function shiftInt(minimal, limit) {
  const op = this.shift();
  return op ? op.toInt(minimal, limit) : -1;
};

ScriptWriter.prototype.removeInt = function removeInt(index, minimal, limit) {
  const op = this.remove(index);
  return op ? op.toInt(minimal, limit) : -1;
};

ScriptWriter.prototype.setInt = function setInt(index, num) {
  return this.set(index, Opcode.fromInt(num));
};

ScriptWriter.prototype.pushInt = function pushInt(num) {
  return this.push(Opcode.fromInt(num));
};

ScriptWriter.prototype.unshiftInt = function unshiftInt(num) {
  return this.unshift(Opcode.fromInt(num));
};

ScriptWriter.prototype.insertInt = function insertInt(index, num) {
...
ScriptWriter.prototype.getBool = function getBool(index) {
  const op = this.get(index);
  return op ? op.toBool() : false;
};

ScriptWriter.prototype.popBool = function popBool() {
  const op = this.pop();
  return op ? op.toBool() : false;
};

ScriptWriter.prototype.shiftBool = function shiftBool() {
  const op = this.shift();
  return op ? op.toBool() : false;
};

ScriptWriter.prototype.removeBool = function removeBool(index) {
  const op = this.remove(index);
  return op ? op.toBool() : false;
};

ScriptWriter.prototype.setBool = function setBool(index, value) {
  return this.set(index, Opcode.fromBool(value));
};

ScriptWriter.prototype.pushBool = function pushBool(value) {
  return this.push(Opcode.fromBool(value));
};

ScriptWriter.prototype.unshiftBool = function unshiftBool(value) {
  return this.unshift(Opcode.fromBool(value));
};

ScriptWriter.prototype.insertBool = function insertBool(index, value) {
...
ScriptWriter.prototype.getSym = function getSym(index) {
  const op = this.get(index);
  return op ? op.toSymbol() : null;
};

ScriptWriter.prototype.popSym = function popSym() {
  const op = this.pop();
  return op ? op.toSymbol() : null;
};

ScriptWriter.prototype.shiftSym = function shiftSym() {
  const op = this.shift();
  return op ? op.toSymbol() : null;
};

ScriptWriter.prototype.removeSym = function removeSym(index) {
  const op = this.remove(index);
  return op ? op.toSymbol() : null;
};

ScriptWriter.prototype.setSym = function setSym(index, symbol) {
  return this.set(index, Opcode.fromSymbol(symbol));
};

ScriptWriter.prototype.pushSym = function pushSym(symbol) {
  return this.push(Opcode.fromSymbol(symbol));
};

ScriptWriter.prototype.unshiftSym = function unshiftSym(symbol) {
  return this.unshift(Opcode.fromSymbol(symbol));
};

ScriptWriter.prototype.insertSym = function insertSym(index, symbol) {
...
ScriptWriter.prototype.render = function render() {
  let size = 0;
...
ScriptWriter.prototype.compile = function compile() {
  this.script.raw = this.render();
  return this.script;
};
...
function ScriptIterator(script, type) {
  this.br = new BufferReader(script.raw, true);
  this.type = type;
  this.index = 0;
}

ScriptIterator.prototype[Symbol.iterator] = function iterator() {
  return this;
};

ScriptIterator.prototype.next = function next() {
  let value = undefined;
  ...
  switch (this.type) {
    case 0:
      value = Opcode.fromReader(this.br);
      break;
    case 1:
      value = [this.index, Opcode.fromReader(this.br)];
      this.index += 1;
      break;
    case 2:
      value = Opcode.readOp(this.br);
      break;
  }
...
exports = Script;
exports.Reader = ScriptReader;
exports.Writer = ScriptWriter;

exports.read = function read(script) {
  return new ScriptReader(script);
};

exports.write = function write(script) {
  return new ScriptWriter(script);
};
